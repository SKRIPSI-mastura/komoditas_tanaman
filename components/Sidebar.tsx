...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    // Tutup sidebar otomatis saat berganti halaman (pada mobile)
    setIsOpen(false);
  }, [pathname]);

  // Efek untuk mendengarkan aksi toggle sidebar dari header pada mobile
  useEffect(() => {
  ...
  return (
    <>
      <aside className={cn(
        "h-screen w-64 fixed left-0 top-0 bg-white dark:bg-stone-950 flex flex-col py-6 z-50 border-r border-[#006B54]/10 shadow-[4px_0_24px_rgba(0,107,84,0.03)] transition-transform duration-300 md:translate-x-0",
        ...
        <div className="px-4 mt-auto pt-2 pb-4 space-y-3">
          ...
          <div className="bg-[#006B54]/5 dark:bg-stone-900/40 p-4 rounded-2xl border border-[#006B54]/10 relative overflow-hidden">
             <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="w-1.5 h-1.5 bg-emerald-50 rounded-full animate-pulse shadow-[0_0_8px_#10b981]" />
             </div>
          ...
          {isAdmin ? (
            <div className="bg-stone-50 dark:bg-stone-900/30 p-3 rounded-2xl border border-stone-100 dark:border-stone-850">
            ...
            <div className="space-y-2">
              <div className="bg-stone-50 dark:bg-stone-900/30 p-3 rounded-2xl flex items-center space-x-3 border border-stone-100 dark:border-stone-850">
                <div className="w-8 h-8 rounded-full bg-stone-200 dark:bg-stone-850 flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-stone-550 text-base" data-icon="person">person</span>
                </div>
      ...
      {isOpen && (
        <div 
          ...
          className="fixed inset-0 bg-stone-950/40 backdrop-blur-sm z-40 md:hidden animate-fade-in"
        />
      )}
    </>
  );
